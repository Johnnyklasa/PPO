...
int main() {
   ...
   char fSend_ID = 1;
   char fSend_OK = 1;
	Decoder MyDecoder(2, (sizeof(asKeywords) / sizeof(asKeywords[0])), asKeywords);
  ...
  UART_Init();
   ...
   while (1) {
      ...
      if (READY == eUART_Rx_GetStatus()) {
         ...
         MyDecoder.DecodeMsg(acRxString);

         if ((MyDecoder.ucTokenNr != 0) && (MyDecoder.pasTokens[0].eType == KEYWORD)) {
            switch (MyDecoder.pasTokens[0].uValue.eKeywordCode) {
               ...
               case GOTO:
                  MyServo.GoTo(MyDecoder.pasTokens[1].uValue.uiNumber);
                  fSend_OK = 1;
                  break;
